...
long EtherCAT_configuration(void)
{
	...
	for(i=0;i<g_NUM_OF_SLAVES;i++)
	{
		...
		*/
		...
		SdoWriten(C_DRIVE_BUSID1+i, EPOS4_FOLLOWING_ERROR_WINDOW, 0, 4, 200000);	// Floowing Error window
		SdoWriten(C_DRIVE_BUSID1+i, EPOS4_MAX_MOTOR_SPEED, 0, 4, 14000);
		SdoWriten(C_DRIVE_BUSID1+i, EPOS4_MAX_PROFILE_VELOCITY, 0, 4, 12500);
		SdoWriten(C_DRIVE_BUSID1+i, EPOS4_PROFILE_VELOCITY, 0, 4, 12000);			// Profile Velocity
		SdoWriten(C_DRIVE_BUSID1+i, EPOS4_PROFILE_ACCELERATION, 0, 4, 10000);
		SdoWriten(C_DRIVE_BUSID1+i, EPOS4_PROFILE_DECELERATION, 0, 4, 10000);
	}
